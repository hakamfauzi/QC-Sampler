...
if uploaded_file is not None:
    ...
    try:
        ...
        # Hitung proporsi efektif jika menggunakan MOE/CL
        N = len(df)
        if mode == "Margin of Error":
            z_lookup = {"90%": 1.645, "95%": 1.96, "99%": 2.576}
            z = z_lookup[confidence_level]
            n_req = compute_sample_size(N, margin_of_error, z)
            proporsi_efektif = (n_req / N) if N > 0 else 0
            st.info(f"Target ukuran sampel: {n_req} dari {N} baris (proporsi {proporsi_efektif:.2%}).")
        else:
            proporsi_efektif = proporsi
            st.info(f"Proporsi sample manual: {proporsi_efektif:.2%} dari {N} baris.")
        ...
        if st.button("🚀 Generate Sample"):
            ...
            with st.spinner("Sedang memproses sampling... Mohon tunggu."):
                try:
                    # 4. Panggil fungsi sampling
                    sample_df = process_sampling(df, group_column, proporsi_efektif, min_sample)
                    ...
                    # 5b. Ringkasan per kategori: Populasi, Target, Sampel Aktual
                    def _target_per_kategori(n_cat):
                        return min(n_cat, max(int(n_cat * proporsi_efektif), min_sample))

                    # Populasi per kategori
                    pop_counts = df[group_column].value_counts()
                    pop_df = pop_counts.reset_index()
                    pop_df.columns = ["Kategori", "Populasi"]

                    # Target sampel per kategori (sebelum sampling)
                    pop_df["Target Sampel"] = pop_df["Populasi"].apply(_target_per_kategori)

                    # Sampel aktual per kategori (setelah sampling)
                    actual_counts = sample_df[group_column].value_counts()
                    actual_df = actual_counts.reset_index()
                    actual_df.columns = ["Kategori", "Sampel Aktual"]

                    ringkasan_df = pop_df.merge(actual_df, on="Kategori", how="left")
                    ringkasan_df["Sampel Aktual"] = ringkasan_df["Sampel Aktual"].fillna(0).astype(int)
